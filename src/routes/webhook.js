// src/routes/webhook.js - FINAL CLEAN VERSION (Zapier Only)
 ...
 * Main webhook endpoint for CSV ingestion (used by Zapier)
...
router.post('/ingest', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    console.error('\n❌ Error in webhook ingestion:', error);
  ...
  res.json({
    ...
    endpoints: {
      ingest: {
        ...
        description: 'Upload CSV file for processing with UID generation',
        contentType: 'multipart/form-data',
        parameters: {
          file: 'CSV file (required)',
          source: 'Source name (optional, defaults to filename)',
          propertyNameColumn: 'Column name for property (optional, default: "Property Name")',
          unitColumn: 'Column name for unit (optional, default: "Unit")',
        },
        example: 'curl -X POST http://your-server/api/webhook/ingest -F "file=@data.csv"',
      },
      health: {
        method: 'GET',
